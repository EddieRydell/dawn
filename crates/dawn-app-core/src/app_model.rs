...
impl AppModel {
    ...
    pub fn refresh_project_entries(&mut self) -> Result<(), String> {
        self.project_root = self.workspace.project_root_display().map(ToString::to_string);
    ...
    fn active_sequence_source(&self) -> Option<(SequenceKey, dawn_project::document::SequenceDocument)> {
        let path = self.editors.active_file()?.clone();
        let overlays = self.editors.dirty_overlays();
        let descriptor = self.workspace.inspect_document(path.clone(), overlays.clone()).ok()?;
