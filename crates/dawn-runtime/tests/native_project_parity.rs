...
fn native_effects_match_reference_dsl_in_real_project_frames() {
    ...
    let native = PreparedSequenceRenderer::prepare(&native_project, setup, sequence).unwrap();
    ...
    let native =
        PreparedSequenceRenderer::prepare(&native_project, setup, &automated_sequence_id).unwrap();
    let reference =
        PreparedSequenceRenderer::prepare(&reference_project, setup, &automated_sequence_id)
            .unwrap();
    assert_eq!(
        native.render_frame(7150).unwrap(),
        reference.render_frame(7150).unwrap()
    );
