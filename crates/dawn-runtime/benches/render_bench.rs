...
use dawn_runtime::{
    PreparedSequenceOutput, PreparedSequenceRenderer, RenderedFrame, SequenceOutputScratch,
    SequenceRenderScratch,
};
...
fn bench_render(c: &mut Criterion) {
    ...
    let mut scenario_scratch = SequenceRenderScratch::default();
    c.bench_function("render_representative_frames", |b| {
        b.iter(|| {
            for scenario in SCENARIOS {
                black_box(
                    renderer
                        .render_frame_with_scratch(black_box(scenario.frame), &mut scenario_scratch)
    ...
    let mut output_scratch = SequenceOutputScratch::default();
    c.bench_function("controller_output_dense_60_frames", |b| {
        b.iter(|| {
            for frame in PLAYBACK_START_FRAME..PLAYBACK_START_FRAME + PLAYBACK_FRAME_COUNT {
                ...
                black_box(
                    output
                        .sample_into(black_box(sample_time), &mut output_scratch)
...
fn assert_scenarios(renderer: &PreparedSequenceRenderer) {
    for scenario in SCENARIOS {
        ...
        assert_eq!(
            renderer.active_effect_count(scenario.frame),
