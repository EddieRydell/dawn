...
use crate::document::{
    ...
    SequenceEffectPixelDocument, SequenceEffectRenderDocument, SequenceMarkCollectionDocument,
};
...
use crate::frame::{ceil_frame, floor_frame, frame_count, frame_start};
use crate::model::{
    Color, Curve, CurveValue, CurveValueType, Distance, DistanceSpan, EffectParam, EffectScriptId,
    FixtureId, Resolved, SequenceEffectScope, Time, TimeSpan,
};
use crate::path::{resolve_import_path, Utf8PathBuf};
use crate::render::{layout_render_plan, GeometryRenderBounds, GeometryRenderPoint};
...
fn is_generator_effect(analysis: &ProjectAnalysis, effect: &SequenceEffectDocument) -> bool {
    effect
        .render
        .as_ref()
...
fn sample_prepared_effect(
    ...
    for pixel in target_pixels {
        ...
        match EffectSampler::new(script, prepared_params, scratch).sample(
            progress,
            local_seconds,
...
fn sample_prepared_effect_thumbnail_column(
    ...
    for pixel in target_pixels {
        ...
        let color = EffectSampler::new(script, prepared_params, scratch)
            .sample(
                progress,
                local_seconds,
                pixel.fixture_context,
...
fn prepare_child_generator_topology(
    statements: &[crate::effect_script::Stmt],
...
struct EffectSampler<'a> {
    script: &'a CompiledEffect,
    params: &'a PreparedEffectParams,
    scratch: &'a mut EffectSampleScratch,
}

impl<'a> EffectSampler<'a> {
    fn new(
        script: &'a CompiledEffect,
        params: &'a PreparedEffectParams,
        scratch: &'a mut EffectSampleScratch,
    ...
    fn sample(
        ...
        self.script.sample_prepared_with_scratch(
            progress,
            seconds,
            fixture,
            pixel,
