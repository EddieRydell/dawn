...
use deprecated_dawn_backend::{
    ActiveGuiDocument, AppView, AudioPlaybackStatus, BufferExternalState, BufferTab,
    EditorViewMode, FixtureGuiEdit as RuntimeFixtureGuiEdit, LayoutGuiEdit as RuntimeLayoutGuiEdit,
    LiveOutputReadout, RuntimeStatus, SequenceGuiEdit as RuntimeSequenceGuiEdit,
    SequenceMarkRef as RuntimeSequenceMarkRef, SequencePasteAnchor as RuntimeSequencePasteAnchor,
    SequenceResizeEdge as RuntimeSequenceResizeEdge, SequenceSelection as RuntimeSequenceSelection,
    SequenceSelectionEdit as RuntimeSequenceSelectionEdit,
    SequenceSelectionEditResult as RuntimeSequenceSelectionEditResult,
};
...
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
#[serde(rename_all = "camelCase")]
pub struct AppBackendChangedDto {
    pub snapshot: AppSnapshotDto,
    pub changed_slices: Vec<BackendSliceDto>,
}

#[derive(Debug, Clone, Copy, Serialize, Deserialize, Type)]
#[serde(rename_all = "camelCase")]
pub enum BackendSliceDto {
    Workspace,
    Editor,
    ActiveDocument,
    Diagnostics,
...
impl From<RuntimeStatus> for RuntimeStatusDto {
    fn from(status: RuntimeStatus) -> Self {
        match status {
            status if status.is_no_project_open() => Self::NoProjectOpen,
            status if status.is_saved() => Self::Saved,
            status => Self::Message {
...
impl From<SequenceGuiEditDto> for RuntimeSequenceGuiEdit {
    fn from(edit: SequenceGuiEditDto) -> Self {
        match edit {
            SequenceGuiEditDto::SetAudio { import } => Self::SetAudio { import },
            ...
            SequenceGuiEditDto::ChangeEffectScript { id, script } => Self::ChangeEffectScript {
                id,
                script: script.into(),
            },
            SequenceGuiEditDto::DeleteEffect { id } => Self::DeleteEffect { id },
            SequenceGuiEditDto::RetargetEffect { id, target } => Self::RetargetEffect {
                id,
                target: target.into(),
            },
            SequenceGuiEditDto::SetEffectScope { id, scope } => Self::SetEffectScope {
                id,
                scope: scope.into(),
            },
            SequenceGuiEditDto::UpdateEffectParam { id, name, value } => Self::UpdateEffectParam {
                id,
                name,
                value: value.into(),
            },
            ...
            SequenceGuiEditDto::DeleteMarkCollection { key } => Self::DeleteMarkCollection { key },
...
impl From<SequenceSelectionDto> for RuntimeSequenceSelection {
...
impl From<RuntimeSequenceSelection> for SequenceSelectionDto {
    fn from(selection: RuntimeSequenceSelection) -> Self {
        match selection {
            RuntimeSequenceSelection::Effects { ids } => Self::Effects { ids },
            RuntimeSequenceSelection::Marks { marks } => Self::Marks {
                marks: marks.into_iter().map(Into::into).collect(),
            },
        }
    }
...
impl From<SequenceMarkRefDto> for RuntimeSequenceMarkRef {
...
impl From<RuntimeSequenceMarkRef> for SequenceMarkRefDto {
    fn from(mark: RuntimeSequenceMarkRef) -> Self {
        Self {
            collection_key: mark.collection_key,
            index: mark.index,
        }
...
impl From<SequencePasteAnchorDto> for RuntimeSequencePasteAnchor {
...
impl From<SequenceResizeEdgeDto> for RuntimeSequenceResizeEdge {
...
impl From<SequenceSelectionEditDto> for RuntimeSequenceSelectionEdit {
...
impl From<RuntimeSequenceSelectionEditResult> for SequenceSelectionEditResultDto {
    fn from(result: RuntimeSequenceSelectionEditResult) -> Self {
        Self {
            selection: result.selection.map(Into::into),
...
impl TryFrom<LayoutGuiEditDto> for RuntimeLayoutGuiEdit {
...
impl TryFrom<FixtureGuiEditDto> for RuntimeFixtureGuiEdit {
    type Error = &'static str;
