...
export const commands = {
	...
	setTerminalPanelLayout: (layout: TerminalPanelLayoutDto) => typedError<AppSnapshotDto, string>(__TAURI_INVOKE("set_terminal_panel_layout", { layout })),
	createTerminalSession: (profile: TerminalProfileDto, cols: number, rows: number, outputChannel: Channel<TerminalEventDto>) => typedError<number, string>(__TAURI_INVOKE("create_terminal_session", { profile, cols, rows, outputChannel })),
	writeTerminalInput: (sessionId: number, data: string) => typedError<null, string>(__TAURI_INVOKE("write_terminal_input", { sessionId, data })),
	resizeTerminalSession: (sessionId: number, cols: number, rows: number) => typedError<null, string>(__TAURI_INVOKE("resize_terminal_session", { sessionId, cols, rows })),
	killTerminalSession: (sessionId: number) => typedError<null, string>(__TAURI_INVOKE("kill_terminal_session", { sessionId })),
...
export type AppSnapshotDto = {
	...
	terminalPanelLayout: TerminalPanelLayoutDto,
};
...
export type TerminalEventDto = { type: "output"; sessionId: number; data: string } | { type: "exited"; sessionId: number; exitCode: number } | { type: "error"; sessionId: number; message: string };

export type TerminalPanelLayoutDto = {
	visible: boolean,
	widthPx: number,
	activeTabIndex: number,
	tabProfiles: TerminalProfileDto[],
};

export type TerminalProfileDto = "powerShell" | "cmd";
