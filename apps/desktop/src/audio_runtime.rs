...
use dawn_backend::{AudioPlaybackStatus, PreviewAudioClock};
...
impl AudioRuntime {
    pub fn play(
        ...
        position_seconds: f64,
    ) -> Result<PreviewAudioClock, String> {
    ...
    pub fn pause(&self) -> Result<PreviewAudioClock, String> {
        let mut inner = self.lock_inner()?;
        inner.poll_load_results();
    ...
    pub fn stop(&self, position_seconds: f64) -> Result<PreviewAudioClock, String> {
        validate_position_seconds(position_seconds)?;
        let mut inner = self.lock_inner()?;
        inner.poll_load_results();
        inner.pending_play_seconds = None;
    ...
    pub fn clock(&self) -> Result<PreviewAudioClock, String> {
        let mut inner = self.lock_inner()?;
        inner.poll_load_results();
        Ok(inner.clock())
    }
...
impl AudioRuntimeInner {
    ...
    fn clock(&mut self) -> PreviewAudioClock {
