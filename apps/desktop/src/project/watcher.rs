...
use crate::app::runtime::emit_runtime_read_models;
            ...
            loop {
                match rx.recv_timeout(debounce) {
                    ...
                    Err(RecvTimeoutError::Timeout) => {
                        ...
                        let snapshot = {
                            let Ok(mut runtime) = crate::app::state::lock_runtime(&state) else {
                                continue;
                            };
                            if runtime.handle_filesystem_changes(paths).is_err() {
                                continue;
                            }
